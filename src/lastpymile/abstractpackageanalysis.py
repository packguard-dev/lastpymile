...
class AbstractPackageAnalysis(ABC):
  ...
  def __init__(self, pyPackage:PyPackage, **options) -> None:
    ...
    if cache_folder is not None:
      version=pyPackage.getVersion()
      folder_name=pyPackage.getName() if (version is None or version=="unknown") else pyPackage.getName()+"_"+version
      cache_folder=os.path.join(cache_folder,folder_name)
  ...
  def startAnalysis(self) -> map[str:Any]:
    ...
    self.__logger.info("Package {} Analysis TERMINATED in {} seconds".format(self.pyPackage.getName(),analysis_report.getAnalysisDurationMs()/1000))
    return analysis_report.getReport()
  ...
  def __doAnalysis(self, analysis_report:AnalysisReport) ->None:
    ...
    self.__logger.info("Package '{}' Analysis STARTED".format(self.pyPackage.getName()))
    ...
    ###
    try:
      self.__logger.info("Sources processing for package '{}' STARTED".format(self.pyPackage.getName()))
    ...
    for release in releases:
      ...
      try:
        ...
        # If the analysis result carries inline statistics, promote them to the report's statistics list
        if "statistics" in result:
          release_stats=StageStatisticsData("analysis_{}".format(release.getReleaseFileName()))
          for key, value in result.pop("statistics").items():
            release_stats.addStatistic(key, value)
          release_stats.stageCompleted()
          analysis_report.addStatistics(release_stats)
  ...
  def __setupTempFolder(self, root_tmp_folder:str) -> str:
    ...
    if root_tmp_folder==None:
      ...
      import time
      version=self.pyPackage.getVersion()
      version_suffix="" if (version is None or version=="unknown") else "_"+Utils.sanitizeFolderName(version)
      tmp_folder= os.path.join(root_tmp_folder,"lpm_"+(str(round(time.time() * 1000)).zfill(10))+"_"+Utils.sanitizeFolderName(self.pyPackage.getName(),20)+version_suffix)
  ...
  class AnalysisReport():
    ...
    def __init__(self,pyPackage):
      ...
      version=pyPackage.getVersion()
      pkg_info={"name":pyPackage.getName()} if (version is None or version=="unknown") else {"name":pyPackage.getName(),"version":version}
